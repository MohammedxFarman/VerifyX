...
export const GeminiService = {
  ...
  async verifyImage(base64Data: string, mimeType: string, fileName?: string, fileSize?: number): Promise<VerifyImageResponse> {
    ...
    try {
      ...
      const textPart = {
        text: `Examine this image for TWO separate things: (1) whether it is a real photograph that has been manipulated (deepfake, photoshop, copy-paste stitching, edge blurring, lighting mismatches, EXIF tampering), AND (2) whether the entire image was originally generated by an AI image tool (ChatGPT/DALL-E, Midjourney, Stable Diffusion, Gemini, etc.) rather than being a real photo or human-made graphic at all.
For (2), weigh signs such as: overly smooth/plastic textures, generic stock-icon-style illustrations, impossible or garbled small text, uncanny symmetry, AI-typical color gradients, or a composition style typical of AI image generators. Vector art, infographics, and illustrations CAN still be AI-generated — the absence of camera noise or EXIF data is not evidence of authenticity for this category, it is simply inconclusive for that specific check.
Context information: ${fileContext}
CRITICAL: Only assign a high Authenticity Score (85-100) and LOW Risk Level if the image shows NO signs of manipulation AND NO signs of being AI-generated in the first place. If the image appears to be entirely AI-generated content, Authenticity Score must be LOW (under 30) and AI Probability HIGH (above 0.7), even if no "tampering" of a real photo is detected. Return a structured response complying exactly with the JSON schema.`
      };
      ...
      return await runWithRetry(async (modelName) => {
        const response = await ai.models.generateContent({
          ...
          config: {
            ...
            systemInstruction: "You are an elite expert in visual forensics, image tampering detection, AND AI-generated image detection. These are two distinct skills: detecting edits to real photos, and recognizing whole images produced by AI generators (ChatGPT, DALL-E, Midjourney, Stable Diffusion). Do not assume an image is authentic just because it lacks camera-photo artifacts like EXIF or noise — illustrations and infographics can still be fully AI-generated. Separate metrics (Authenticity, AI Probability, Manipulation Score, Confidence, Risk Level) independently. Respond in JSON strictly adhering to the schema.",
            responseMimeType: "application/json",
            responseSchema: {
              type: Type.OBJECT,
              properties: {
